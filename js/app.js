(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            ...
            new swiper_core_Swiper(".bestseller__slider", {
                ...
                navigation: {
                    prevEl: ".bestseller__arrow--prev",
                    nextEl: ".bestseller__arrow--next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1.1,
            ...
            new swiper_core_Swiper(".deals__slider", {
                modules: [ Autoplay ],
                observer: true,
                observeParents: true,
                speed: 800,
    ...
    for (const filter of filters) filter.addEventListener("click", (function(e) {
        e.preventDefault();
        const catId = filter.getAttribute("data-filter");
        const itemCat = document.querySelectorAll(".item-product");
        itemCat.forEach((function(c) {
            if (catId === "all") c.classList.remove("hide"); else if (c.getAttribute("data-cat") !== catId) c.classList.add("hide"); else c.classList.remove("hide");
        }));
    }));
    const filterItem = document.getElementsByClassName("filter__item");
    const actives = document.getElementsByClassName("active");
    for (let i = 0; filterItem.length > i; i++) filterItem[i].onclick = function() {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (timer) {
            ...
            (function timer() {
                const deadline = new Date(Date.now() + (4e4 * 60 * 1e3 + 999));
